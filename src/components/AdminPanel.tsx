...
// Marcador EN VIVO desde el estadio: +/- rápido, con minuto y goleadores. No reparte puntos.
export function LiveScoreAdmin({ matches }: { matches: MatchOption[] }) {
  ...
  const push = (nh: number, na: number, min = minute, sc = scorers) => {
    setH(nh); setA(na)
    if (sel === '') return
    startTransition(async () => {
      const r = await setLiveScore(sel as number, nh, na, min, sc)
  ...
  return (
    <section className="card mx-[18px] mb-4" style={{ borderColor: 'var(--red)' }}>
      ...
      <p className="text-xs font-bold mb-3" style={{ color: 'var(--muted)' }}>
        Desde el estadio: marcador con +/−, minuto y goleadores. Aparece EN VIVO sobre la foto del estadio en Pronósticos (sin repartir puntos aún).
      </p>
      <select
        className="input w-full mb-3"
        value={sel}
        onChange={(e) => { const id = Number(e.target.value) || ''; setSel(id); setH(0); setA(0); setMinute(''); setScorers(''); setMsg('') }}
      >
      ...
      {m && (
        <>
          ...
          <input className="input w-full mb-2" placeholder="Minuto (ej. 67' · MT · 90+3')" value={minute} onChange={(e) => setMinute(e.target.value)} />
          <input className="input w-full mb-2" placeholder="Goleadores (ej. Quiñones 9', Jiménez 67')" value={scorers} onChange={(e) => setScorers(e.target.value)} />
          <button disabled={busy} onClick={() => push(h, a)} className="savebtn" style={{ background: 'var(--red)' }}>
            {busy ? 'ACTUALIZANDO…' : '🔴 ACTUALIZAR EN VIVO'}
          </button>
          <p className="text-[11px] font-bold mt-2" style={{ color: 'var(--muted)' }}>Cuando termine, usa “Resultado manual” abajo para finalizar y repartir los puntos 🏁</p>
        </>
      )}
      {msg && <p className="text-xs font-bold mt-2 text-center" style={{ color: 'var(--ink)' }}>{msg}</p>}
    </section>
  )
}
